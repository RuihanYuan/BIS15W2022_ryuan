...
```{r}
life_history%>%
  purrr::map_df(~sum(is.na(.)))#map to data frame
```
...
life_history<-life_history%>%
  na_if("-999")%>%
  mutate(newborn=na_if(newborn,("not measured")))#replace not measured with na
```
...
```{r}
naniar::miss_var_summary(life_history)#include percent missing
```
...
life_history <- 
  life_history %>% 
  mutate(max_life=na_if(max_life, 0))#replce 0 with na
```
...
```{r}
trade_wildlife<-readr::read_csv("data/cites.csv")%>%clean_names()
```

2. Use `naniar` to summarize the NA's in each variable.
```{r}
naniar::miss_var_summary(trade_wildlife)
```
...
trade_wildlife%>%
  ...
  naniar::miss_var_summary()%>% #no need for order=T
...
```{r}
vis_miss(life_history)#only focusing on the na(w/%)
```
