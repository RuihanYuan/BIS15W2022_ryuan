...
penguins %>% 
  group_by(island) %>% 
  summarize(number_NAs=sum(is.na(body_mass_g))) #how many NAs for each by group
```

Well, that won't work so let's remove the NAs and recalculate.
```{r}
penguins %>% 
  filter(!is.na(body_mass_g)) %>% #take out NA(ALWAYS check if NA exist first!!)
  group_by(island) %>% 
  summarize(mean_body_mass_g=mean(body_mass_g),
...
```{r}
penguins %>% 
  tabyl(island, species)#for making report
```
...
penguins%>%
  ...
  summarize(m_bill_length_mm=mean(bill_length_mm,na.rm=T),
            m_bill_depth_mm=mean(bill_depth_mm,na.rm=T),
            m_flipper_length_mm=mean(flipper_length_mm,na.rm=T),
            m_body_mass_g=mean(body_mass_g,na.rm=T),
            n_sample=n())
```
```{r}
...
penguins%>%
  ...
  group_by(species)%>%
  summarize(mean_bill_length_mm = mean(bill_length_mm)) #or have na.rm=T
```
...
penguins%>%
  group_by(species)%>%
  summarise(mean_blength_mm=mean(bill_length_mm,na.rm=T),
            mean_bdepth_mm=mean(bill_depth_mm,na.rm=T),
            mean_flength_mm=mean(flipper_length_mm,na.rm=T),
            n_sample=n())
```
```{r}
penguins%>%
  group_by(species)%>%
  summarise(sd_blength_mm=sd(bill_length_mm,na.rm=T),
            sd_bdepth_mm=sd(bill_depth_mm,na.rm=T),
            sd_flength_mm=sd(flipper_length_mm,na.rm=T),
            n_sample=n())
```
## Wrap-up  
