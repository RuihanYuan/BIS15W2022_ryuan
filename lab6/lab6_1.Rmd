...
```{r}
msleep%>% count(vore) #count how many vores for each
```
...
msleep%>%
  ...
  filter(n_vores>1) #come up with only ones with more than one vores
```
...
msleep%>%
  ...
  summarize(mean_bodywt_p=mean(bodywt),
            ...
            total=n())#total can also write as "n_samples"
...
names(msleep)
```
There are some NAs occur in the data but not all. It just concluded to NA.
...
```
Have the NaN because we don't have any data.
