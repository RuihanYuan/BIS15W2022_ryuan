...
```{r}
ggplot(data=iris, mapping=aes(x=Species, y=Petal.Length))#need geom
```
...
ggplot(data=homerange, mapping=aes(x=log10.mass, y=log10.hra)) +
  geom_point()+
  geom_smooth(method=lm, se=T) #adds the regression(best-fit) line, `se=TRUE` will add standard error
...
```{r}
homerange %>% 
  count(trophic.guild)#count=group_by &summarize
```
...
homerange %>% 
  ...
  geom_col()#use col bc have both x and y
```
```{r}
homerange %>% 
  filter(family=="salmonidae") %>% 
