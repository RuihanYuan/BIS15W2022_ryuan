...
1. The `ui` function controls the user inputs and the way the app will be displayed.  #overall look of app
...
```{r, eval=F, echo=T}
ui <- fluidPage(       #name of dropdown menu
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), #drop-down menu
              selected = "bill_length_mm")#where drop down menu start
)
...
ui <- fluidPage(
  ...
  plotOutput("plot", width = "500px", height = "400px") #display a plot; give dimension
)
...
```{r}
library(shiny) #since only adjust fill, just one input
