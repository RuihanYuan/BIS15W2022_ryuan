...
ui <- fluidPage(
    ...
    #step:how far pointsize being segmented
    plotOutput("plot", width = "500px", height = "400px") #slider on point size
)
...
server <- function(input, output, session) {
  ...
  ggplot(penguins, aes_string(x = input$x, y = input$y, color="species")) + 
      geom_point(size=input$pointsize) + #where to put the point size
      theme_light()
    })
}
