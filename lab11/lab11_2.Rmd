...
```{r}
p+theme_linedraw()+ #add a under layer
...
```
```{r}
p+theme_minimal()+ #add a under layer
  theme(axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
       y= "n",
...
#economist(newspaper "economist" use)
```
...
```{r}
my_palette <- paletteer_d("calecopal::superbloom1")#make it default palette for subs plot
```
