...
```{r}
deserts$year <- as.factor(deserts$year)#bc R see year as dbl(numeric value)
```
...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=2)+ #shape number can be different
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "chartreuse", bins=40)+ #bins can be determined by R, no                                        #outline color       #inside color         need to specify
...
homerange %>% 
  ...
  geom_density(color = "red")+ #density line, need to specify y
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "gold", bins=40)+ 
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(fill="sienna2", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Homerange")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "plum", alpha = 0.4, color = "black")+ #alpha:transparency
  geom_density(color = "sienna2")+ 
  labs(title = "Distribution of Homerange")
```
...
#the case when log10.mass is greater or equal to 1.75 consider as small
#now mass_category is chr in the data
```
...
homerange <- homerange%>%
  mutate(range_category = case_when(log10.hra < 3.65 ~ "very small",
                                    log10.hra >= 3.65 &log10.mass<4.59 ~"small",
                                    log10.hra >= 4.59 & log10.mass < 6.02 ~ "medium",
...
homerange%>%
  ...
  labs(title="Observation of Range Category in Homerange Data",
       x="Range Category")
```
...
homerange%>%
  ...
  ggplot(aes(x=class,y=log10.mass,fill=range_category))+
