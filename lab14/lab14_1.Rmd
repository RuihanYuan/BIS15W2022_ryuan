...
homerange%>%
  ...
  geom_boxplot(alpha=0.4,color="grey",na.rm=T)+
  geom_violin(alpha=0.6,na.rm = T)+
  theme_linedraw()+
  coord_flip()
```
...
heartrate_diff <- heartrate%>%
  mutate(across(c(a,b,c,d), ~ . - no_drug)) %>% #subtract heartrate of no drug
  select(-no_drug) %>% 
    pivot_longer(-patient, #patient does not pivot
               names_to = "drug", 
               values_to = "heartrate"
