...
spiders #date being read as <date>
```
...
spiders %>% 
  ...
  summary()#find min and max
```

Now we set the bounding box.
```{r}
lat <- c(34.67, 41.80)#(min,max)
long <- c(-124.1, -115.5)#(min,max)
bbox <- make_bbox(long, lat, f = 0.05)#make those coordinates a little bigger
```
...
County boundaries in each state.（add layers to the map)
```{r}
...
ggplot() +
  geom_sf(data = ca_comp, size = 0.125)+
  geom_sf(data = ca_cnty_comp, size = 0.125)+
  geom_point(data = spiders, aes(longitude, latitude), size=0.8)+#above:take the same map
