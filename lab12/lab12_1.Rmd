...
```{r}
files <- list.files(path = "data/spiders", pattern = ".csv", full.names = TRUE)#tells where the docs are
files
```
...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv)#looping csv over on every file
```

We can view elements in our list with double brackets. Let's view the data for Butte county.
```{r}
spider_list[[3]] #pull element 3 out
class(spider_list[[3]]) #tells the class of that element
```
```{r}
spider_list[[2]]
class(spider_list[[2]])
...
```
```{r}
str(spider_list[[2]]) #make sure to use double bracket
```
...
```{r}
names_list <- strsplit(names, split = " .csv") #split with " .csv"
names_list
```

```{r}
names_vec <- unlist(names_list)#make all names to a vector
names_vec
```
...
```{r}
spiders_all <- bind_rows(spider_list)#match all columns by name
spiders_all
```
...
```{r}
#join_type(firstTable, secondTable, by=columnTojoinOn) #by sth that are the same for both table
```
...
```{r}
inner_exampleDF <- inner_join(table_A, table_B, by="customer_ID") #if not mtach both, it will drop 
inner_exampleDF
```
...
```{r}
left_exampleDF <- left_join(table_A, table_B, by="customer_ID")#joining the first data frame, no customer 4
left_exampleDF
```
...
```{r}
right_exampleDF <- right_join(table_A, table_B, by="customer_ID")#match table B, no customer 3
right_exampleDF
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID")#puts everything together
full_exampleDF
```
...
```{r}
anti_exampleDF <- anti_join(table_A, table_B, by="customer_ID")#provide data missing in the second table,what not in                                                                  both table
anti_exampleDF
```
...
```{r}
datetime <- now()
datetime#what the day, time now
```
...
```{r}
dmy(spiders_with_locs$Date)#dmy=day, month, year
```
...
```{r}
write.csv(spiders_with_locs, file = "spiders_with_locs.csv", row.names = FALSE)#remove row names
```
