...
```{r}
select(fish, fish_id:length)##from fish_is to length
```
...
```{r}
select_if(fish, is.numeric)##allow to choose specific data that fit a particular class
```
...
```{r}
mammals_new<-rename(mammals, genus="Genus", wean_mass="wean mass", max_life="max. life", litter_size="litter size", litter_size="litter size", litter_per_year="litters/year")
mammals_new
```
