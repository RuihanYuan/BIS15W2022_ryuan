...
2. How does the age of elephants compare by sex?(asking a range)
```{r}
elephants%>%
...
#hjust:adjust the position of the title
#rel:relative size
```
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+geom_bar()+#fill color by sex
...
homerange%>%
  ...
  labs(title = "Ectothemrs vs. Endotherms",
       x = "thermorgulation",
       y = "n") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
```{r}
homerange%>%
  ggplot(aes(x=log10.mass,y=log10.hra))+geom_point(size=1)+ #adjust point size
  labs(title = "Mass vs. Homerange",
       x = "log10.mass",
       y = "log10.hra") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
